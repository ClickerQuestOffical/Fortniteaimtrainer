...
export function makeScenario(test,settings,seed=1){const r=seeded(seed);const targets=[];let yaw=0,pitch=0;const angular=[0,15,30,45,60,90,120,135,150,180];for(let i=0;i<(test.targets||1);i++){
 ...
 const dist=1; yaw+=Math.cos(a*Math.PI/180)*dist;pitch+=Math.sin(a*Math.PI/180)*dist;targets.push({id:i,spawnTime:i*900+300,spawnYaw:a%360,spawnPitch:p,radius:rad,velocity:vel,direction:a,type:test.type==='trackX'||test.type==='trackY'?'moving':'static'});
 }return targets}
